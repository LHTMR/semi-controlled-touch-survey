...
def load_theme_lexicon(theme_dir: str) -> Dict[str, Set[str]]:
    # Try enhanced JSON first (wordnet_cluster_X_enhanced.json), then plain JSON, then CSV fallback
    import glob

    originals, synonyms, closest = set(), set(), set()

    # Look for enhanced JSON files in the folder
    enhanced_files = glob.glob(os.path.join(theme_dir, "*_enhanced.json"))
    json_files = glob.glob(os.path.join(theme_dir, "*.json"))
    csv_path = os.path.join(theme_dir, "wordnet_selections_extended.csv")

    if enhanced_files or json_files:
        # Prefer enhanced JSON, fall back to any JSON
        json_path = enhanced_files[0] if enhanced_files else json_files[0]

        with open(json_path, "r", encoding="utf-8") as f:
            data = json.load(f)

        # Format: {word: [{synset, synonyms, closest_words, closest_words_info}, ...]}
        for word, synset_list in data.items():
            ...
            for entry in synset_list:
                if not isinstance(entry, dict):
                    continue

                # Synonyms from WordNet lemmas (stored in "synonyms" field)
                ...
                # Closest words
                for c in entry.get("closest_words", []):
                    c = c.strip()
                    if not c:
                        continue
        ...
        raise FileNotFoundError(f"No lexicon file found in: {theme_dir}")
