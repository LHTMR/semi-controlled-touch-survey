...
DEFAULT_CONFIG = {
    ...
    # Preserves semantic meaning in subjective responses --> example not good does not get split into not and good
    "handle_negations": True,
    "negation_terms": [  # list of strings --> words that count as negations
        "not",
        "no",
        "never",
        "none",
    ...
    "negation_intensifiers": [  # list of strings --> words that stengtehn or modify meaning
        "very",
        "really",
        "quite",
        "rather",
    ...
    # Maximum words between negation and target (linguistic constraint)
    "negation_max_distance": 3,  # integer--> how many words away a negation can supply
    ...
    # Groups morphological variants (e.g., feel/feeling/felt)
    ...
    # Analysis after stopword removal and negation handling
    "save_filtered_counts": True,
    # Detailed grouping information for methodological transparency
    "save_grouped_counts": True,
    # JSON dictionary for custom transformations and human review
    "save_grouping_dict": True,
    # Visual representation of word variation patterns
    "save_tree_visualization": True,
    "tree_max_depth": None,  # Limits tree depth for readability while showing key variations
    # Minimum frequency for words/groups to be included in output
...
# function definition: creates a fucntion taking one inout (config). none means it does not return anyrin. it just does soemthing
def setup_nltk_data(config: Dict[str, Any]) -> None:
    ...
    if config.get("nltk_data_path"):  # if statement + method call --> folder path for NLTK data, then do the following --> get() safely checks if a key exists in the dictionary without crashing if it doesn't
        # method call -->adds that custom folder path to the list of places NTLK will look for its data files
        nltk.data.path.append(config["nltk_data_path"])

    # Required NLTK packages
    required_packages = [  # list variable --> creates a list of NLTK packages that the program needs to work
        # tuple --> a pair: first item is the folder path where it's stored, second is the name used to download it. punkt splits text into words
        ("tokenizers/punkt", "punkt"),
        # same idea--> the updated version of punkt
        ("tokenizers/punkt_tab", "punkt_tab"),
        # the list of filler wprds to remove
        ("corpora/stopwords", "stopwords"),
    ]

    # if the settings say to use lemmatization then do the following
    if config.get("use_lemmatization", True):
        # method call--> adds wordnet to the list of required packages neededfor lemmatization
        required_packages.append(("corpora/wordnet", "wordnet"))

    # for loop--> goes through each package i the list one by one. each item is a tuple so it unpacks it inot two variables (find_path & package_name)
    for find_path, package_name in required_packages:
        try:
            # checks if the package is already installed on the computer
            nltk.data.find(find_path)
            # if found print a sucessfull message
            print(f"✓ {package_name} is available")
        except LookupError:  # if the package was not found, catch that error and do the following
            # print telling the user that the package is missing and is being downloade
            print(f"Downloading {package_name}...")
            try:
                # function call --> downloads the missing package from the internet
                nltk.download(package_name)
                # confirms the download worked
                print(f"✓ {package_name} downloaded successfully")
            except Exception as e:  # if the download failed for any reason, catch the error
                # shows what went wrong
                print(f"✗ Error downloading {package_name}: {e}")
                # suggest a fix
                print("Please check your internet connection and try again.")
                if package_name == "stopwords":  # if the package that failed was stopwords specifically
                    # then turns off stopwords removal seince the required paackage is not available
                    print("Stopword removal will be disabled.")
                    # turns off stopwords removal since the required package is not available
                    config["remove_stopwords"] = False
                elif package_name == "wordnet":  # or if the package that failed was wordnet
                    print("Lemmatization will be disabled.")
                    # turn off lemmatization since wordnet is not available
...
# Creates a function called load_spacy_model. Takes config (the settings) as input. -> Optional[Any] means it will return either a spaCy model object or None if it fails
def load_spacy_model(config: Dict[str, Any]) -> Optional[Any]:
    ...
    """
    if not config.get("use_spacy_negation", True):  # Default is True: if the settings say to not use Spacy, the not flips condition
        return None  # exit the fucntion and run nothing --> spacy won't be loaded

    # if spacy is being used, tells teh user it is loading the model
    print("Loading spaCy model for enhanced negation detection...")

    try:  # attempt teh following and if it fails handle it graefully
        # try to load teh spacy library (inside the fucntion bc spacy might not be installed, so doing it here lets teh prohram handle that gracefully)
        import spacy
        # loads a specific tool from spacy called token --> represents a single word in a asentence
        from spacy.tokens import Token
    except ImportError:  # if spacy is not installed, catch tye error and do the following instead of crashing
        # Tells the user spaCy is missing
        print("✗ spaCy is not installed. Falling back to heuristic negation detection.")
        # gives the user instructions on how to install spacy
        print("  To use spaCy, install it with: pip install spacy")
        # gives the next step downloading the actual english laanguage model
        print("  Then download the model with: python -m spacy download en_core_web_sm")
        # turns off spacy in the settings since it is not available
        config["use_spacy_negation"] = False
        return None  # exits the function immediately returning nothing --> the program will continue without spacy
    ...
    try:
        ...
        print(
            f"  Download the model with: python -m spacy download {model_name}")
    ...
    if head.pos_ == "AUX":
        ...
        for child in head.children:
            ...
            if child.dep_ in ("acomp", "attr"):
                adv_modifiers = [
                    c for c in child.children if c.dep_ == "advmod"]
...
def process_text_with_spacy(text: str, nlp: Any, config: Dict[str, Any]) -> list[str]:
    ...
    for token in doc:
        if token.lemma_ == "absent" and token.dep_ == "acomp":
            subj = next(
                (c for c in token.head.children if c.dep_ == "nsubj"), None)
            if subj:
        ...
        if token.text in negation_terms:
            ...
            if target_tokens:
                ...
                if token.dep_ == "advmod" and token in filtered_target_tokens:
                    lemmatized_targets = [
                        t.lemma_ for t in filtered_target_tokens]
                ...
                for t in filtered_target_tokens:
                    ...
                    tokens_to_skip.update(
                        c.i for c in get_full_compound_spacy(t))
...
def detect_file_format(
    ...
    if format_override:
        print(
            f"  Using specified format: {format_override} (bypassing detection)")
        ...
        if format_override == "csv":
            ...
            try:
                ...
                print(
                    f"  Warning: Could not detect delimiter for CSV override: {e}")
            ...
            try:
                ...
                if len(lines) >= 3:
                    ...
                    for delimiter in config.get(
                        ...
                        delimiter_in_all = all(
                            delimiter in line for line in lines[:3])
                        if delimiter_in_all:
                            # Check consistency
                            counts = [line.count(delimiter)
                                      for line in lines[:3]]
...
def extract_text_from_dataframe(df: pd.DataFrame, config: Dict[str, Any]) -> List[str]:
    ...
    if text_columns:
        ...
        missing_columns = [
            col for col in text_columns if col not in df.columns]
        ...
        if missing_columns:
            ...
            if unmatched_columns:
                print(
                    f"   Columns not found (will be skipped): {unmatched_columns}")
                ...
                if not text_columns:
                    ...
                    raise ValueError(
                        "No specified text columns found in dataframe")
        else:
    ...
    if not text_columns:
        ...
        for col in df.columns:
            try:
                ...
                string_ratio = (
                    string_count /
                    len(sample_values) if len(sample_values) > 0 else 0
                )
                ...
                column_analysis.append(
                    (col, 0.0, 0.0, f"error: {str(e)[:50]}", False))
    ...
    if not text_columns:
        ...
        if conversion_errors:
            ...
            if len(conversion_errors) > 3:
                print(
                    f"      ... and {len(conversion_errors) - 3} more errors")
    ...
    for col_idx, col in enumerate(text_columns):
        ...
        try:
            ...
            try:
                ...
                col_text = (
                    df[col].apply(lambda x: str(
                        x) if not pd.isna(x) else "").tolist()
                )
                ...
                extraction_stats.append(
                    (col, 0, total_rows, "failed", str(e2)[:50]))
                continue
        ...
        avg_length = (
            sum(len(text.strip())
                for text in col_text if text.strip()) / non_empty
            if non_empty > 0
            else 0
        )

        extraction_stats.append(
            (col, non_empty, empty, method_used, "success"))
    ...
    for col, non_empty, empty, method, status in extraction_stats:
        if status == "success":
            print(
                f"      ✓ {col}: {non_empty} non-empty, {empty} empty ({method})")
        else:
    ...
    empty_pct_total = (empty_count / len(all_text) *
                       100) if len(all_text) > 0 else 0
...
def extract_raw_words(text: str, config: Dict[str, Any]) -> List[str]:
    ...
    try:
        ...
        print(
            f"    ⚠️  Unexpected error in extract_raw_words: {str(e)[:50]}...")
        ...
        if use_spacy:
            try:
                ...
                if nlp is not None:
                    ...
                    spacy_tokens = process_text_with_spacy(
                        original_text, nlp, config)
                ...
                print(
                    f"  ⚠️  Error in spaCy negation processing: {str(e)[:50]}...")
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    print(
                        f"    ⚠️  All tokenization methods failed: {str(e3)[:50]}...")
...
def is_negative_stopword(
    ...
    if not spacy_terms:
        ...
        spacy_terms = {"not", "n't", "no",
                       "never", "seldom", "rarely", "without"}
...
def load_custom_grouping_dict(path: str) -> Dict[str, List[str]]:
    """
    Load a custom grouping dictionary from a JSON file.

    Accepts two formats:
      1. Direct: {"main_word": ["variant1", "variant2", ...], ...}
      2. Wrapped (output from save_grouping_dictionary):
         {"metadata": {...}, "transformation_dictionary": {"main_word": [...], ...}}

    Returns:
        Dict mapping main_word -> list of variants (including main_word itself).
    """
    ...
    if "transformation_dictionary" in data:
        return data["transformation_dictionary"]

    # Handle direct format — filter out any non-dict values just in case
    return {k: v for k, v in data.items() if isinstance(v, list)}


def group_words_by_custom_dict(
    word_counts: Dict[str, int], custom_dict: Dict[str, List[str]]
) -> Dict[str, List[Tuple[str, int]]]:
    """
    Group word_counts using a pre-defined custom dictionary.

    Words not found in the dictionary are kept as their own single-member group.

    Args:
        word_counts: Dictionary of word frequencies from the text.
        custom_dict: Dict mapping main_word -> [variant1, variant2, ...]

    Returns:
        Dictionary: {main_word: [(variant, count), ...]}
    """
    print("Grouping words using custom dictionary...")

    # Build a reverse lookup: variant -> main_word
    variant_to_main = {}
    for main_word, variants in custom_dict.items():
        for variant in variants:
            variant_to_main[variant.lower()] = main_word.lower()
        # Also map main_word to itself
        variant_to_main[main_word.lower()] = main_word.lower()

    groups: Dict[str, List[Tuple[str, int]]] = {}
    ungrouped = 0

    for word, count in word_counts.items():
        main = variant_to_main.get(word.lower())
        if main:
            groups.setdefault(main, []).append((word, count))
        else:
            # Word not in custom dict — keep as its own group
            groups.setdefault(word, []).append((word, count))
            ungrouped += 1

    print(f"  Groups formed from custom dict: {len(groups) - ungrouped}")
    print(f"  Words not in custom dict (kept as-is): {ungrouped}")
...
def group_word_variations(
    ...
    print(
        f"  Created {len(groups)} word groups from {len(word_counts)} unique words")
    return groups
    ...
    for main_word, members in sorted_groups:
        ...
        if len(members) == 1:
            ...
            if isinstance(max_depth, int):
                # Show up to max_depth members
                display_members = members[:max_depth]
            else:
            ...
            for i, (word, count) in enumerate(display_members):
                ...
                tree_lines.append(
                    f"    {prefix}{word} ({count}, {percentage:.1f}%)")

            # If there are more members than max_depth, show a summary line
            if remaining_count > 0:
                tree_lines.append(
                    f"    └── ... and {remaining_count} more variations")
...
def filter_groups_by_threshold(
    ...
    if removed > 0:
        print(
            f"  Filtered out {removed} groups with total frequency < {threshold}")
...
def update_config_from_args(
    ...
    if args.format:
        ...
        print(
            f"Using specified format: {args.format} (bypassing automatic detection)")
...
Examples:
  ...
  %(prog)s data.unknown --format csv  # Force CSV format for unknown file extension
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to a custom grouping dictionary JSON file. "
             "Format: {main_word: [variant1, variant2, ...]}. "
             "When provided, skips automatic similarity-based grouping.",
    )
    parser.add_argument(
    ...
    print(
        f"Extracted {len(processed_words)} processed words (with spaCy if available)")
    ...
    if config.get("save_raw_counts", True) and raw_word_counts:
        save_word_outputs(
            raw_word_counts, config["output_prefix"], "raw", config)
    ...
    # Group word variations — use custom dictionary if provided, else auto-detect
    custom_dict_path = getattr(args, "custom_dict", None)
    if custom_dict_path:
        try:
            print(
                f"\nLoading custom grouping dictionary from: {custom_dict_path}")
            custom_dict = load_custom_grouping_dict(custom_dict_path)
            print(f"  Loaded {len(custom_dict)} groups from custom dictionary")
            groups = group_words_by_custom_dict(
                filtered_word_counts, custom_dict)
        except Exception as e:
            print(
                f"⚠️  Could not load custom dictionary ({e}). Falling back to auto-grouping.")
