...
def find_closest_words(
    ...
    for i, synset in enumerate(all_synsets):
        ...
        distance = synset_distance(
            reference_synset, synset, method=similarity_method)
...
def parse_multiple_selections(selection_input, max_selection):
    ...
    # Split by commas
    parts = [part.strip()
             for part in selection_input.split(",") if part.strip()]
...
def disambiguate_words(
    ...
    for i, word in enumerate(words):
        ...
        if INTERRUPTED:
            ...
            if selections:
                try:
                    ...
                    print(
                        colored(f"✗ Could not save partial results: {e}", "red"))
            return selections
        ...
        if selection_input.upper() == "STOP":
            ...
            if selections:
                try:
                    save_partial_results(selections, output_dir)
                    print(
                        colored(f"✓ Saved {len(selections)} selections", "green"))
                except Exception as e:
        ...
        if not selection_input and past_selections_indices:
            ...
            selected_indices = parse_multiple_selections(
                selection_input, len(synsets))
        ...
        if safe_mode and selections:
            try:
                ...
                print(
                    colored(f"Note: Could not save checkpoint: {e}", "yellow"))
...
def save_results(
        ...
        for synset_name in synset_names:
            try:
                ...
                if word.lower() in [s.lower() for s in synonyms]:
                    synonyms = [s for s in synonyms if s.lower() !=
                                word.lower()]
                ...
                if extend_synonyms:
                    ...
                    closest_words = [word_info[0]
                                     for word_info in closest_words_info]
                ...
                if extend_synonyms:
                    csv_extended_rows.append(
                        [
                            ...
                            ", ".join(synonyms) if synonyms else "N/A",
                            ", ".join(
                                closest_words) if closest_words else "N/A",
                            (
                                ", ".join(
                                    [
                                        f"{info[0]} ({info[1]:.3f})"
    ...
    # Save enhanced JSON
    enhanced_json_path = os.path.join(
        output_dir, "wordnet_selections_synonyms.json")
    ...
    if extend_synonyms and csv_extended_rows:
        csv_extended_path = os.path.join(
            output_dir, "wordnet_selections_extended.csv")
...
def load_previous_results(filepath, validate=True, force_resume=False):
    ...
    try:
                            ...
                            if choice == "2":
                                ...
                                if synsets:
                                    ...
                                    if (
                                        ...
                                        replacement = synsets[int(
                                            sel) - 1].name()
                                        if word in valid_selections and isinstance(
                                            valid_selections[word], list
                                        ):
                                            valid_selections[word].append(
                                                replacement)
                                        else:
                                            valid_selections[word] = [
                                                replacement]
                else:
                    # Single synset
                    try:
                    ...
                    if not force_resume:
                        ...
                        if choice == "2":
                            ...
                            if synsets:
                                ...
                                # Show first 5
                                for j, ss in enumerate(synsets[:5], 1):
                                    print(
                                        f"  {j}. {ss.definition()} ({ss.name()})")
        ...
        if force_resume:
            ...
            if recovered:
                print(
                    f"Recovered {len(recovered)} entries from corrupted file")
                return recovered
  ...
  %(prog)s --resume corrupted_results.json --force-resume --no-validate
    ...
    if args.words:
        ...
        try:
            ...
            print(
                colored(f"Error: Input file not found: {args.input_file}", "red"))
            return 1
    else:
        # Interactive mode
